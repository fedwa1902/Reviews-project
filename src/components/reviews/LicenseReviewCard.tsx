...
import type { LicenseReviewItem } from '@/types/review';

interface LicenseReviewCardProps {
  review: LicenseReviewItem;
  index: number;
  onDecision: (id: string, decision: 'keep' | 'release', comment?: string) => void;
}
...
const decisionOptions: LicenseDecisionOption[] = [
  {
    value: 'keep',
    label: 'Keep License',
    description: 'I still actively use this license',
  ...
  },
  {
    ...
    description: 'This license can be reassigned to someone else',
...
export function LicenseReviewCard({ review, index, onDecision }: LicenseReviewCardProps) {
  const [selectedDecision, setSelectedDecision] = useState<'keep' | 'release' | null>(null);
  const [comment, setComment] = useState('');
  const [showComment, setShowComment] = useState(false);

  const isCompleted = review.status !== 'pending';
  const isStale = new Date().getTime() - new Date(review.lastUsedDate).getTime() > 90 * 24 * 60 * 60 * 1000;
          ...
          <div className="text-right">
            {isCompleted ? (
              <Badge className="bg-success text-success-foreground border-0">
                ...
                {review.decision === 'keep' ? 'Keeping' : 'Releasing'}
              </Badge>
            ) : (
              <div className="flex items-center gap-1 text-lg font-heading font-bold text-foreground">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 text-sm">
            ...
            <div>
              ...
              <span className={`font-medium ${isStale ? 'text-warning-foreground' : ''}`}>
                {formatRelativeDate(review.lastUsedDate)}
                {isStale && ' ⚠'}
              </span>
            </div>
          </div>
            ...
            <Tooltip>
              <TooltipTrigger>
                <div className="flex items-center gap-1.5 rounded-full bg-primary/10 px-3 py-1 text-xs font-medium text-primary">
                  ...
                  ${review.costPerMonth * 12}/year
                </div>
              </TooltipTrigger>
              <TooltipContent>
                <p>Annual cost projection if the license is kept</p>
              </TooltipContent>
            </Tooltip>
          )}
        </div>
          ...
          <div className="space-y-4 pt-4 border-t border-border/60">
            ...
            <div>
              <p className="text-sm font-medium mb-3 text-foreground">
                Do you still need this license?
              </p>
                ...
                {decisionOptions.map((option) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Cost hint for release option */}
                      {option.value === 'release' && !isSelected && (
                        <Badge variant="secondary" className="text-[10px] px-1.5 py-0 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 shrink-0">
                          Save ${review.costPerMonth}/mo
                        </Badge>
                      )}
                    </button>
                  );
